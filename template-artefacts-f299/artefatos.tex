...
\hline
\textbullet~ Clínicas de neuropsicologia e psiquiatria 
\newline 
\textbullet~ Profissionais da saúde mental autônomos 
\newline 
\textbullet~ Instituições de reabilitação cognitiva
&
\textbullet~ Desenvolver um serious game que avalia atenção sustentada, impulsividade e tempo de resposta com base no TDC \newline \textbullet~ Devolver um feedback do resultado do teste de cada usuário
&
\textbullet~ Computador com câmera e acesso a internet \newline \textbullet~ Hospedagem em servidor
&
\textbullet~ Testes Gamificados e Interativos \newline \textbullet~ Geração de um Pré\textbullet~diagnóstico rápido \newline \textbullet~ Layout Simples e Intuitivo
&
\textbullet~ Anúncios na internet \newline \textbullet~ Website \newline \textbullet~ Disponibilidade para Download \newline \textbullet~ Recomendações de outros usuários \\
\hline
& \textbf{6. Recursos chave} & & \textbf{7. Atividades chave} & \\
\hline
& \textbullet~ Software de uso fácil \newline \textbullet~ Teste Gamificado e Interativo \newline \textbullet~ Feedback gamificado para o usuário final \newline \textbullet~ Conveniencia de ter um pré\textbullet~diagnóstico \newline \textbullet~ Alta interatividade \newline \textbullet~ Relatórios automatizados com interpretação de resultados
& &
\textbullet~ Criação de conteúdo educacional \newline \textbullet~ Desenvolvimento de software \newline \textbullet~ Manutenção do sistema \newline \textbullet~ Suporte ao usuário &
\\
\hline
\multicolumn{2}{|p{5.5cm}|}{\textbf{8. Parceiros chave}} & \multicolumn{3}{p{9.4cm}|}{\textbf{9. Estrutura de custos}} \\
\hline
\multicolumn{2}{|p{5.5cm}|}{\textbullet~ Clínicas de saúde mental \newline \textbullet~ Usuário final} & \multicolumn{3}{p{9.4cm}|}{\textbullet~ Custos fixos: internet, hospedagem do programa, infraestrutura, investimento em tráfego pago \newline \textbullet~ Custos variáveis: Desenvolvimento de software, manutenção de software, custo de marketing e divulgação, patrocínio de profissionais em neuropsicologia e psiquiatria, custo de testes com usuários, custo de recrutamento e formação de usuários, custo de suporte ao usuário} \\
\hline
\end{tabular}
\end{table}
...
No âmbito interno, o projeto destaca-se pelo uso de \textit{eye-tracking} em um ambiente gamificado, o que transforma uma tarefa clínica geralmente exaustiva em uma experiência de alto engajamento, o que aumenta a fidelidade dos dados coletados e reduz a taxa de abandono do teste. 
...
A crescente aceitação da triagem remota abre portas para parcerias B2B (\textit{Business-to-Business}) com clínicas e instituições de ensino, posicionando o \textit{FocusQuest} como uma solução de baixo custo e alta escalabilidade. 

Entretanto, o projeto opera em um setor altamente regulado. A conformidade com a \textbf{LGPD (Lei Geral de Proteção de Dados)} e as normas de saúde representam ameaças estruturais que exigem atenção técnica e jurídica. Além disso, a possível entrada de grandes concorrentes exige que o projeto mantenha agilidade na inovação e foco na experiência do usuário para preservar sua diferenciação de mercado.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Inovação:} Diferencial competitivo com uso de \textit{eye-tracking} acessível.
    \item \textbf{Engajamento:} Metodologia de \textit{Serious Game} que reduz o abandono em testes clínicos.
    \item \textbf{Rigor Científico:} Alinhamento técnico com o protocolo TDC.
\end{itemize}
\end{minipage}
&
\begin{minipage}[t]{7cm}
...
\begin{minipage}[t]{7cm}
\begin{itemize}[leftmargin=*]
    \item \textbf{Expansão Digital:} Crescente demanda por ferramentas de triagem remota.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Concorrência:} Entrada de grandes concorrentes com maior capacidade de investimento.
\end{itemize}
\end{minipage}
\\
\hline
...
\begin{itemize}
    \item \textbf{Estratégias Ofensivas (Forças + Oportunidades):} Explorar o eye-tracking para posicionar o produto como líder no nicho de triagem remota, utilizando a gamificação como diferencial de mercado frente a métodos tradicionais.
...
\hline
Controle de Versão & Git + GitHub & \textbullet~ Versionamento do código-fonte e acionamento de pipelines \\
Execução de aplicações & Cloud Run & \textbullet~ Hospedagem serverless de frontend e backend com auto scaling e HTTPS \\
Containerização & Docker & \textbullet~ Empacotamento do frontend e backend \\
Balanceamento de carga & Cloud Load Balancer & \textbullet~ Distribuição de tráfego, TLS/SSL e suporte a WebSocket \\
CDN & Cloud CDN & \textbullet~ Cache global para assets estáticos (imagens, CSS, JS)  \\
Cache / Mensageria & Memorystore for Redis & \textbullet~ Cache em tempo real e comunicação distribuída entre instâncias \\
Banco de dados & MongoDB Atlas & \textbullet~ Persistência principal dos dados da aplicação \\
Armazenamento & Cloud Storage & \textbullet~ Armazenamento de backups e logs com versionamento e recuperação \\
Gerenciamento de segredos & Secret Manager & \textbullet~ Armazenamento seguro de credenciais e variáveis sensíveis \\
Monitoramento & Cloud Monitoring e Logging  & \textbullet~ Observabilidade de CPU, memória, latência e centralização de logs \\
CI/CD & Cloud Build & \textbullet~ Automação de build, push de imagens e deploy contínuo \\
Rede & VPC Network & \textbullet~ Isolamento de rede e comunicação privada entre serviços \\
Provisionamento & Scripts shell & \textbullet~ Setup da rede, Redis, Artifact Registry e deploy \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=*, nosep]
    ...
    \item Redis para cache, sessão e sincronização em tempo real
...
\begin{itemize}
    \item \textbf{Usuários:} Acessam a aplicação direcionados primeiramente por um balanceador de carga externo.
    \item \textbf{Balanceamento de Carga:} Cloud Load Balancing é utilizado em duas camadas (tráfego externo para a camada Web e tráfego interno da camada Web para a camada de Aplicação).
    \item \textbf{Web Tier (Front End):} Instâncias executadas em contêineres utilizando o Cloud Run.
    \item \textbf{Application Tier (Back End):} Serviços de retaguarda e API também executados de forma serverless através do Cloud Run.
    \item \textbf{Cache Estático (CDN):} Cloud CDN utilizado na camada Web para entrega rápida e otimizada de assets estáticos.
    \item \textbf{Cache de Aplicação:} Redis utilizado na camada de Back End para otimização de performance e redução de latência.
    \item \textbf{Banco de Dados:} MongoDB (hospedado de forma externa no Mongo Atlas) comunicando-se diretamente com o Back End.
    \item \textbf{Storage / Backups e Logs:} Cloud Storage utilizado de forma centralizada para armazenar os logs e backups tanto do Front End quanto do Back End.
    \item \textbf{Rede / Infraestrutura:} Ambiente contido dentro do Google Cloud Platform (GCP), isolado em uma VPC e sub-rede na região us-central1.
\end{itemize}
